...
var LastClearTime;
//track id image http://images-aws.shazam.com/webtid/11227140/size/160
...
//    osg.log("secs " + secs);
    jQuery("#TweetContent").html("<img id=\"closeTweet\" src=\"img/close.png\" alt=\"close\"><a href=\"" + tweet.url + "\" target=\"_blank\"><img src=\"img/wiki-globe.png\" alt=\"\"></a>" + "<a href=\"" + tweet.url +  "\" target=\"_blank\"><span id=\"username\">" + tweet.user.screen_name + " </span></a> <p><a href=\""+tweet.url+"\">" + text + "</a></p> <span id=\"date\">" + msg + "</span>" + tweet.parsedcomment );
    console.log(tweet.parsedcomment);
    //jQuery("#TweetContent").attr("data-inid", );
...
//    scene.addChild(twitterTagsItems);
    ...
    var getCountPerSecond = function() {
        jQuery("#tweetsPersecond").html(countPerSecond);
        countPerSecond = 0;
    };

    setInterval(getCountPerSecond, 1000);
    ...
    var consumeTweet = function(tweet) {
        ...
        var displayTweet = function(location, tweet) {
            ...
            if (lat !== 0.0 && lng !== 0.0) {
                ...
                var scale = Viewer.getManipulator().scale * 70.0;
                ...
                var imageUrl = "img/wiki.png";

                //Filter a particular tag by the song name
                if (tweet.text && tweet.text.toLowerCase().match('like a virgin')) {
                    console.log(tweet);
		...
		//test
		//var server_url = "/globeTweeter/mine/imageData.php?url=" + escape(tweet.user.profile_image_url);
		//var server_url = "http://192.168.0.7:3000/?url=" + escape(tweet.user.profile_image_url);
        //$.getJSON(server_url, function(data) {
			
			//tweet.user.profile_image_url = data.data;
